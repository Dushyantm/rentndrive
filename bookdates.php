...
<!-- Sidebar/menu -->
<nav class="w3-sidebar w3-bar-block w3-white w3-collapse w3-top" style="width:13%" id="mySidebar">
  ...
  <div class="w3-padding-64 w3-large w3-text-grey">
    ...
    <a href="addcar.html" class="w3-bar-item w3-button">Add Car</a>
...
<div class="w3-row-padding w3-light-grey w3-padding-64 w3-container" style="margin-left:16%">
  ...
  <div class="w3-content">
    ...
    <div class="w3-twothird">
      <form  method="POST" >
          ...
          <input class="w3-input" type="date" placeholder="from date" required name="from_date">
          <input class="w3-input " type="date" placeholder="to date" required name="return_date">
          ...
          <?php 
              ...
              $qry = "SELECT * FROM `booking_details` WHERE from_date BETWEEN '$f' AND '$t' ";
               $result = mysqli_query($conn, $qry);
      echo "<table class='w3-table w3-bordered'>";  
      echo'<th>Booking ID</th><th>Car ID</th><th>Cust ID</th><th>From date</th>'; 
      while($data = mysqli_fetch_array($result))
      {

      echo'<tr>'; 
      echo '<td>'.$data['booking_id'].'</td><td>'.$data['car_id'].'</td><td>'.$data['cust_id'].'</td><td>'.$data['from_date'].'</td>'; 
      echo'</tr>'; 
      }
  ...
  <div class="w3-black w3-center w3-padding-24">WDL Project     </div>
